...
from src.data.models import (
    Notification, ExternalService, InternalSystem,
    NotificationStatusEnum, # Removed NotificationSourceType, NotificationPriority
    ProcessingStatusEnum, RawEmail, LLMData, NotificationTypeEnum, SeverityEnum # Added Enums
)

# Helper to create an external service. Ensures uniqueness for test runs.
def _ensure_external_service(db: Session, name: str = "Test SP for Notif CRUD") -> ExternalService:
    existing = crud.get_external_service_by_name(db, name)
    if existing:
        return existing
    return crud.create_external_service(db, service_name=name, provider="Test Provider Co", description="Acts as a service provider for notifications")

# Helper to create an internal system. Ensures uniqueness for test runs.
def _ensure_internal_system(db: Session, name: str = "Test IS for Notif Link CRUD") -> InternalSystem:
    existing = crud.get_internal_system_by_name(db, name)
    if existing:
        return existing
    return crud.create_internal_system(db, system_name=name, responsible_contact=f"{name.replace(' ', '_')}@example.com", description="IS for notification link testing")
...
def basic_notification_from_email_factory(db_session: Session):
    ...
    def _factory(suffix: str = ""):
        timestamp_suffix = str(datetime.now().timestamp()).replace('.', '') # Ensure unique email ID
...
def test_create_notification_from_email(db_session: Session, basic_notification_from_email_factory):
    suffix = "_create"
    subject = f"Test Email Subject {suffix}"
    original_email_id = f"test_email_id_{suffix}{str(datetime.now().timestamp()).replace('.', '')}"
    ...
    assert db_notification.llm_data.processing_status == ProcessingStatusEnum.UNPROCESSED
    ...
    assert db_notification.llm_data.notification_type == NotificationTypeEnum.UNKNOWN # Default value
...
def test_get_notification(db_session: Session, basic_notification_from_email_factory):
    ...
    fetched_notif = crud.get_notification(db=db_session, notification_id=created_notif.id)
...
def test_get_notifications(db_session: Session, basic_notification_from_email_factory):
    ...
    notifications_page = crud.get_notifications(db=db_session, skip=0, limit=initial_count + 10) # Get all, with some buffer
    ...
    all_current_notifs_sorted_by_creation = sorted(crud.get_notifications(db_session, limit=2000), key=lambda x: x.created_at, reverse=True)
...
def test_get_notification_by_original_email_id(db_session: Session):
    ...
    fetched_notif = crud.get_notification_by_original_email_id(db_session, unique_email_id)
    assert fetched_notif is not None
    assert fetched_notif.id == created_notif.id
    assert fetched_notif.raw_email_data.original_email_id_hash == hashlib.sha256(unique_email_id.encode()).hexdigest()

def test_get_notification_by_original_email_id_not_found(db_session: Session):
    fetched_notif = crud.get_notification_by_original_email_id(db_session, "non_existent_email_id_12345")
    assert fetched_notif is None
...
def test_update_llm_data_extracted_fields_success(db_session: Session, basic_notification_from_email_factory):
    ...
    assert updated_llm_data.event_start_time.year == start_time.year # Approximate datetime check
    assert updated_llm_data.event_end_time.year == end_time.year   # Approximate datetime check
    ...
    assert notification.status == NotificationStatusEnum.TRIAGED # Mapped from COMPLETED
...
def test_update_llm_data_extracted_fields_llm_data_not_found(db_session: Session):
    updated_llm_data = crud.update_llm_data_extracted_fields(
        ...
        llm_summary="N/A", raw_llm_response="{}",
        processing_status=ProcessingStatusEnum.COMPLETED
    )
    assert updated_llm_data is None

def test_update_llm_data_status_to_error(db_session: Session, basic_notification_from_email_factory):
    ...
    assert notification.status == NotificationStatusEnum.ERROR_PROCESSING # Mapped from ERROR

def test_update_llm_data_status_to_manual_review(db_session: Session, basic_notification_from_email_factory):
...
def test_update_llm_data_status_to_pending_validation(db_session: Session, basic_notification_from_email_factory):
    """Tests updating LLMData status to PENDING_VALIDATION and checks notification status mapping."""
    notification = basic_notification_from_email_factory("_update_llm_status_pending_val")
    assert notification.llm_data is not None
    llm_data_id = notification.llm_data.id
    ...
    db_session.refresh(notification)
    assert notification.status == NotificationStatusEnum.PENDING_VALIDATION # Mapped from PENDING_VALIDATION
...
def test_get_pending_notifications(db_session: Session, basic_notification_from_email_factory):
    ...
    crud.update_llm_data_extracted_fields(
        db_session, done_notif.llm_data.id, "Some Service", None, None, 
        NotificationTypeEnum.INFO, SeverityEnum.LOW, "Summary", "{}", ProcessingStatusEnum.COMPLETED
    )
    ...
    crud.update_llm_data_status(db_session, validation_notif.llm_data.id, ProcessingStatusEnum.PENDING_VALIDATION)
