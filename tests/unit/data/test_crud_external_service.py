...
def _create_internal_system(db: Session, name: str = "Test System") -> InternalSystem:
    ...
    return crud.create_internal_system(db, system_name=name, responsible_contact="test_contact@example.com", description="A test internal system")

# Helper to create a dependency, needed for testing dependency checks
def _create_dependency(db: Session, internal_system_id: int, external_service_id: int) -> Dependency:
    return crud.create_dependency(db, internal_system_id=internal_system_id, external_service_id=external_service_id, dependency_description="A test dependency")
...
def test_create_external_service(db_session: Session):
    ...
    db_service = crud.create_external_service(db=db_session, service_name=service_name, provider=provider, description=description)
...
def test_create_external_service_already_exists(db_session: Session):
    ...
    crud.create_external_service(db=db_session, service_name=service_name) # Create first time
    
    # Attempt to create again
    db_service_again = crud.create_external_service(db=db_session, service_name=service_name)
    
    assert db_service_again is not None
    assert db_service_again.service_name == service_name
    
    services: List[ExternalService] = db_session.query(ExternalService).filter(ExternalService.service_name == service_name).all()
    assert len(services) == 1 # create_external_service should return existing if name matches

def test_get_external_service(db_session: Session):
    service = crud.create_external_service(db=db_session, service_name="Test Service Gamma")
...
def test_get_external_service_by_name(db_session: Session):
    ...
    fetched_service = crud.get_external_service_by_name(db=db_session, service_name=service_name)
    assert fetched_service is not None
    assert fetched_service.service_name == service_name

def test_get_external_service_by_name_not_found(db_session: Session):
    fetched_service = crud.get_external_service_by_name(db=db_session, service_name="NonExistentService")
    assert fetched_service is None
...
def test_get_external_services(db_session: Session):
    ...
    s1 = crud.create_external_service(db=db_session, service_name="Service Epsilon Unique")
    ...
    all_services_ordered = sorted(crud.get_external_services(db=db_session, skip=0, limit=1000), key=lambda x: x.id)
    if len(all_services_ordered) > 1:
        services_skip_1_limit_1 = crud.get_external_services(db=db_session, skip=1, limit=1)
...
def test_update_external_service(db_session: Session):
    service = crud.create_external_service(db=db_session, service_name="Service Eta", provider="OldProvider")
    assert service is not None
    ...
    updated_service = crud.update_external_service(db=db_session, service_id=service.id, **update_data)
...
def test_update_external_service_name_conflict(db_session: Session):
    crud.create_external_service(db=db_session, service_name="Existing Name ForConflictTest")
    service_to_update = crud.create_external_service(db=db_session, service_name="Original Name ForConflictTest")
    
    updated_service = crud.update_external_service(db=db_session, service_id=service_to_update.id, service_name="Existing Name ForConflictTest")
...
def test_update_external_service_not_found(db_session: Session):
    updated_service = crud.update_external_service(db=db_session, service_id=88888, service_name="Doesn't Matter")
...
def test_update_external_service_no_changes(db_session: Session):
    ...
    service = crud.create_external_service(db=db_session, service_name=service_name_theta, provider=provider_theta)
    assert service is not None

    # Try to update with the same data or no data
    updated_service = crud.update_external_service(db=db_session, service_id=service.id, provider=provider_theta)
    assert updated_service is not None
    ...
    updated_service_no_args = crud.update_external_service(db=db_session, service_id=service.id)
...
def test_delete_external_service(db_session: Session):
    ...
    service = crud.create_external_service(db=db_session, service_name=service_name_iota)
    assert service is not None
...
def test_delete_external_service_with_dependencies(db_session: Session):
    ...
    ext_service = crud.create_external_service(db=db_session, service_name=ext_service_name)
    ...
    db_session.refresh(ext_service) # Refresh to load the 'dependencies' relationship if needed by CRUD
    ...
    fetched_service = crud.get_external_service(db=db_session, service_id=ext_service.id)
    assert fetched_service is not None
