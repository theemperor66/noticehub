...
def _create_external_service(db: Session, name: str = "Test Ext Service For IS Test") -> ExternalService:
    return crud.create_external_service(db, service_name=name, provider="Test Provider", description="Test desc for IS test helper")
...
def _create_dependency(db: Session, internal_system_id: int, external_service_id: int) -> Dependency:
    return crud.create_dependency(db, internal_system_id=internal_system_id, external_service_id=external_service_id, dependency_description="Test dep for internal system test")
...
def test_create_internal_system(db_session: Session):
    ...
    db_system = crud.create_internal_system(db=db_session, system_name=system_name, responsible_contact=responsible_contact, description=description)
...
def test_create_internal_system_already_exists(db_session: Session):
    ...
    db_system_again = crud.create_internal_system(db=db_session, system_name=system_name)
    assert db_system_again is not None
    assert db_system_again.system_name == system_name
    systems: List[InternalSystem] = db_session.query(InternalSystem).filter(InternalSystem.system_name == system_name).all()
...
def test_get_internal_system_by_name(db_session: Session):
    ...
    fetched_system = crud.get_internal_system_by_name(db=db_session, system_name=system_name_delta)
    assert fetched_system is not None
    assert fetched_system.system_name == system_name_delta

def test_get_internal_system_by_name_not_found(db_session: Session):
    fetched_system = crud.get_internal_system_by_name(db=db_session, system_name="NonExistentSystemName IS")
    assert fetched_system is None

def test_get_internal_systems(db_session: Session):
    initial_systems_count = len(crud.get_internal_systems(db=db_session, skip=0, limit=1000))
    ...
    all_systems_ordered = sorted(crud.get_internal_systems(db=db_session, skip=0, limit=1000), key=lambda x: x.id)
    if len(all_systems_ordered) > 1:
        systems_skip_1_limit_1 = crud.get_internal_systems(db=db_session, skip=1, limit=1)
...
def test_update_internal_system(db_session: Session):
    ...
    system = crud.create_internal_system(db=db_session, system_name=system_name_eta_orig, responsible_contact="old_contact_is@example.com")
    assert system is not None
    update_data = {
    ...
    updated_system = crud.update_internal_system(db=db_session, system_id=system.id, **update_data)
...
def test_update_internal_system_name_conflict(db_session: Session):
    ...
    system_to_update = crud.create_internal_system(db=db_session, system_name=original_name_update)
    updated_system = crud.update_internal_system(db=db_session, system_id=system_to_update.id, system_name=existing_name)
    assert updated_system is None

def test_update_internal_system_not_found(db_session: Session):
    updated_system = crud.update_internal_system(db=db_session, system_id=99902, system_name="Doesn't Matter System IS")
    assert updated_system is None
...
def test_update_internal_system_no_changes(db_session: Session):
    ...
    system = crud.create_internal_system(db=db_session, system_name=system_name_theta, responsible_contact=contact_theta)
    assert system is not None
    updated_system = crud.update_internal_system(db=db_session, system_id=system.id, responsible_contact=contact_theta)
    assert updated_system is not None
    assert updated_system.system_name == system_name_theta
    assert updated_system.responsible_contact == contact_theta
    updated_system_no_args = crud.update_internal_system(db=db_session, system_id=system.id)
...
def test_delete_internal_system_with_dependencies(db_session: Session):
    ...
    internal_sys = crud.create_internal_system(db=db_session, system_name=internal_sys_name)
