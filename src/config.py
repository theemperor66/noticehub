...
class Settings(BaseSettings):
    ...
    email_check_interval_seconds: int = Field(
        60, validation_alias="EMAIL_CHECK_INTERVAL_SECONDS"
    )

    # Email Filtering Configuration
    email_sender_domain_whitelist: Optional[List[str]] = Field(
        default_factory=list, validation_alias="EMAIL_SENDER_DOMAIN_WHITELIST"
    )
    email_sender_domain_blacklist: Optional[List[str]] = Field(
        default_factory=list, validation_alias="EMAIL_SENDER_DOMAIN_BLACKLIST"
    )
    email_subject_keywords_whitelist: Optional[List[str]] = Field(
        default_factory=list, validation_alias="EMAIL_SUBJECT_KEYWORDS_WHITELIST"
    )
    email_subject_keywords_blacklist: Optional[List[str]] = Field(
        default_factory=list, validation_alias="EMAIL_SUBJECT_KEYWORDS_BLACKLIST"
    )

    @field_validator(
        "email_sender_domain_whitelist",
        "email_sender_domain_blacklist",
        "email_subject_keywords_whitelist",
        "email_subject_keywords_blacklist",
        mode="before",
    )
