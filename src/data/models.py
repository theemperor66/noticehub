from sqlalchemy import create_engine, Column, Integer, String, DateTime, Text, ForeignKey, Enum as SQLAlchemyEnum, Index, UniqueConstraint, Boolean
...
class ProcessingStatusEnum(str, enum.Enum):
    ...
    PENDING_VALIDATION = "pending_validation" # LLM completed, needs human/rule validation
...
class NotificationStatusEnum(str, enum.Enum):
    ...
    ACTION_PENDING = "action_pending"         # Triaged, and specific actions identified or required
    ...
    ERROR_PROCESSING = "error_processing"       # Error during LLM or subsequent processing steps
    PENDING_MANUAL_REVIEW = "pending_manual_review" # Requires human intervention
    PENDING_VALIDATION = "pending_validation"     # Awaiting validation of LLM output or other data
...
class RawEmail(Base):
    ...
    original_email_id_hash = Column(String(64), unique=True, index=True, nullable=False) # SHA256 hash
    ...
    notification = relationship("Notification", back_populates="raw_email_data", uselist=False)
...
class LLMData(Base):
    ...
    processing_status = Column(SQLAlchemyEnum(ProcessingStatusEnum, name="processing_status_enum"), default=ProcessingStatusEnum.UNPROCESSED, nullable=False, index=True)
    error_message = Column(Text, nullable=True)
    raw_llm_response = Column(Text, nullable=True) # Store the full JSON/text response from LLM
    ...
    notification_type = Column(SQLAlchemyEnum(NotificationTypeEnum, name="notification_type_enum"), default=NotificationTypeEnum.UNKNOWN, nullable=True)
    severity = Column(SQLAlchemyEnum(SeverityEnum, name="severity_enum"), default=SeverityEnum.UNKNOWN, nullable=True)
    ...
    # Relationship (one-to-one with Notification)
    notification = relationship("Notification", back_populates="llm_data", uselist=False)
...
class Notification(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    title = Column(String(1024), nullable=True) # Can be derived from email subject or set manually
    status = Column(SQLAlchemyEnum(NotificationStatusEnum, name="notification_status_enum"), default=NotificationStatusEnum.NEW, nullable=False, index=True)
    
    # Foreign Keys to link RawEmail and LLMData
    raw_email_id = Column(Integer, ForeignKey('raw_emails.id'), unique=True, nullable=False, index=True)
    llm_data_id = Column(Integer, ForeignKey('llm_data.id'), unique=True, nullable=False, index=True)

    last_checked_at = Column(DateTime, server_default=func.now(), nullable=False) # When the notification was last reviewed/processed by the system
    created_at = Column(DateTime, server_default=func.now(), nullable=False)
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now(), nullable=False)

    # Relationships (Notification is the owner of the one-to-one)
    raw_email_data = relationship("RawEmail", back_populates="notification", uselist=False, cascade="all, delete-orphan", single_parent=True)
    llm_data = relationship("LLMData", back_populates="notification", uselist=False, cascade="all, delete-orphan", single_parent=True)
...
class Dependency(Base):
    ...
    external_service_id = Column(Integer, ForeignKey('external_services.id'), nullable=False)
    ...
    __table_args__ = (UniqueConstraint('internal_system_id', 'external_service_id', name='_internal_external_uc'),)
    def __repr__(self):
...
def get_db_engine():
    ...
    if engine is None:
        ...
        engine = create_engine(db_url, connect_args=connect_args, echo=settings.db_echo_log)
    return engine
...
def create_tables():
    ...
    try:
        ...
        Base.metadata.create_all(db_engine) # Should handle order correctly for SQLAlchemy
...
if __name__ == '__main__':
    logger.info("Initializing database and creating tables (if run directly from models.py)...")
    create_tables()
    ...
    if engine:
        ...
        if expected_tables.issubset(set(table_names)):
            ...
            logger.warning(f"Missing expected tables. Expected: {expected_tables}, Found: {set(table_names)}")
