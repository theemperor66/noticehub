...
from src.data.models import (
    Base, Notification, ExternalService, InternalSystem, Dependency, 
    RawEmail, LLMData, ProcessingStatusEnum, NotificationTypeEnum, SeverityEnum, NotificationStatusEnum
)
from src.utils.logger import logger

ModelType = TypeVar("ModelType", bound=Base)

def _get_query_with_options(db: Session, model_cls: Type[ModelType], options: Optional[List[Any]] = None) -> Query:
...
def get_item_by_id(db: Session, model: Type[ModelType], item_id: int, options: Optional[List[Any]] = None) -> Optional[ModelType]:
...
def create_notification(
    ...
    try:
        ...
        # 2. Create LLMData
        db_llm_data = LLMData(
            processing_status=ProcessingStatusEnum.UNPROCESSED
        )
        db.add(db_llm_data)
        ...
        logger.info(f"Created Notification ID {db_notification.id} (RawEmail ID: {db_raw_email.id}, LLMData ID: {db_llm_data.id}) for original email hash {hashed_email_id}")
        return db_notification
    except Exception as e:
        db.rollback()
        logger.error(f"Error creating notification for original_email_id_str {original_email_id_str}: {e}", exc_info=True)
        return None

def _map_llm_status_to_notification_status(llm_processing_status: ProcessingStatusEnum) -> NotificationStatusEnum:
...
def update_llm_data_extracted_fields(
    ...
    notification = db.query(Notification).filter(Notification.llm_data_id == llm_data_id).options(joinedload(Notification.llm_data)).first()
    if notification:
        ...
        logger.info(f"Parent Notification ID {notification.id} status updated to {notification.status.value}")
    else:
        logger.warning(f"Could not find parent Notification for LLMData ID {llm_data_id} to update status.")

    try:
        db.commit()
        db.refresh(db_llm_data)
        if notification: db.refresh(notification)
        logger.info(f"Updated LLMData ID {llm_data_id}. Status: {processing_status.value}")
        return db_llm_data
    except Exception as e:
        db.rollback()
...
def update_llm_data_status(
    ...
    raw_llm_response: Optional[str] = None # Store raw response even on error if available
) -> Optional[LLMData]:
    ...
    if notification:
        notification.status = _map_llm_status_to_notification_status(processing_status)
        notification.last_checked_at = datetime.now(timezone.utc)
        logger.info(f"Parent Notification ID {notification.id} status updated to {notification.status.value} due to LLMData status change.")
    else:
        logger.warning(f"Could not find parent Notification for LLMData ID {llm_data_id} to update status.")
    ...
    try:
        ...
        db.refresh(db_llm_data)
        if notification: db.refresh(notification)
        logger.info(f"Updated LLMData ID {llm_data_id} status to {processing_status.value}.")
        return db_llm_data
    except Exception as e:
        db.rollback()
        logger.error(f"Error updating LLMData ID {llm_data_id} status: {e}", exc_info=True)
        return None

def get_notification_by_original_email_id(db: Session, original_email_id_str: str, options: Optional[List[Any]] = None) -> Optional[Notification]:
    hashed_email_id = hashlib.sha256(original_email_id_str.encode()).hexdigest()
    raw_email = db.query(RawEmail).filter(RawEmail.original_email_id_hash == hashed_email_id).first()
    if raw_email:
...
def get_notification(db: Session, notification_id: int, options: Optional[List[Any]] = None) -> Optional[Notification]:
...
def get_pending_notifications(db: Session, limit: int = 10, options: Optional[List[Any]] = None) -> List[Notification]:
    """Retrieves notifications whose LLMData is UNPROCESSED or PENDING_VALIDATION."""
    query = _get_query_with_options(db, Notification, options)
    return query.join(Notification.llm_data).filter(
...
def create_external_service(db: Session, service_name: str, provider: Optional[str] = None, description: Optional[str] = None) -> ExternalService:
    """
    Creates a new external service.
    Checks if a service with the same name already exists.
    """
    existing_service = db.query(ExternalService).filter(ExternalService.service_name == service_name).first()
    if existing_service:
        logger.warning(f"Attempted to create an external service with a name that already exists: '{service_name}' (Existing ID: {existing_service.id}). Returning existing service.")
    ...
    logger.info(f"Created external service '{db_external_service.service_name}' with ID {db_external_service.id}.")
...
def get_external_service_by_name(db: Session, service_name: str) -> Optional[ExternalService]:
    """
    Retrieves an external service by its unique name.
    """
    service = db.query(ExternalService).filter(ExternalService.service_name == service_name).first()
    if not service:
        logger.debug(f"External service with name '{service_name}' not found.")
    return service

def get_external_services(db: Session, skip: int = 0, limit: int = 100) -> List[ExternalService]:
    """
    Retrieves a list of external services with pagination, ordered by service_name.
    """
    return db.query(ExternalService).order_by(ExternalService.service_name).offset(skip).limit(limit).all()
...
def update_external_service(
    ...
    """
    db_external_service = db.query(ExternalService).filter(ExternalService.id == service_id).first()
    ...
    if 'service_name' in update_data_dict and update_data_dict['service_name'] != db_external_service.service_name:
        existing_service_with_new_name = db.query(ExternalService).filter(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Error updating external service ID {service_id}: {e}", exc_info=True)
        return None
...
def delete_external_service(db: Session, service_id: int) -> bool:
    ...
    """
    db_external_service = db.query(ExternalService).options(joinedload(ExternalService.dependencies)).filter(ExternalService.id == service_id).first()
    ...
    try:
        ...
        db.commit()
        logger.info(f"Successfully deleted external service ID {service_id} ('{db_external_service.service_name}').")
        return True
    except Exception as e:
        db.rollback()
        logger.error(f"Error deleting external service ID {service_id}: {e}", exc_info=True)
        return False

# --- InternalSystem CRUD Operations ---

def create_internal_system(db: Session, system_name: str, responsible_contact: Optional[str] = None, description: Optional[str] = None) -> InternalSystem:
    """
    Creates a new internal system.
    Checks if a system with the same name already exists.
    """
    existing_system = db.query(InternalSystem).filter(InternalSystem.system_name == system_name).first()
    if existing_system:
        logger.warning(f"Attempted to create an internal system with a name that already exists: '{system_name}' (Existing ID: {existing_system.id}). Returning existing system.")
    ...
    logger.info(f"Created internal system '{db_internal_system.system_name}' with ID {db_internal_system.id}.")
...
def get_internal_system_by_name(db: Session, system_name: str) -> Optional[InternalSystem]:
    """
    Retrieves an internal system by its unique name.
    """
    system = db.query(InternalSystem).filter(InternalSystem.system_name == system_name).first()
    if not system:
        logger.debug(f"Internal system with name '{system_name}' not found.")
    return system

def get_internal_systems(db: Session, skip: int = 0, limit: int = 100) -> List[InternalSystem]:
    """
    Retrieves a list of internal systems with pagination, ordered by system_name.
    """
    return db.query(InternalSystem).order_by(InternalSystem.system_name).offset(skip).limit(limit).all()
...
def update_internal_system(
    ...
    """
    db_internal_system = db.query(InternalSystem).filter(InternalSystem.id == system_id).first()
    ...
    if 'system_name' in update_data_dict and update_data_dict['system_name'] != db_internal_system.system_name:
        existing_system_with_new_name = db.query(InternalSystem).filter(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Error updating internal system ID {system_id}: {e}", exc_info=True)
        return None
...
def delete_internal_system(db: Session, system_id: int) -> bool:
    ...
    """
    db_internal_system = db.query(InternalSystem).options(joinedload(InternalSystem.dependencies)).filter(InternalSystem.id == system_id).first()
    ...
    try:
        ...
        db.commit()
        logger.info(f"Successfully deleted internal system ID {system_id} ('{db_internal_system.system_name}').")
        return True
    except Exception as e:
        db.rollback()
        logger.error(f"Error deleting internal system ID {system_id}: {e}", exc_info=True)
        return False
...
def get_dependencies(db: Session, skip: int = 0, limit: int = 100) -> List[Dependency]:
    ...
    return db.query(Dependency).options(
        joinedload(Dependency.internal_system),
        joinedload(Dependency.external_service)
    ).order_by(Dependency.id.asc()).offset(skip).limit(limit).all()

def get_dependencies_for_internal_system(db: Session, internal_system_id: int, skip: int = 0, limit: int = 100) -> List[Dependency]:
    ...
    if not internal_system:
        logger.warning(f"Cannot get dependencies: InternalSystem with ID {internal_system_id} not found.")
        return []

    return db.query(Dependency).options(
        joinedload(Dependency.external_service)
    ).filter(Dependency.internal_system_id == internal_system_id).order_by(Dependency.id.asc()).offset(skip).limit(limit).all()

def get_dependencies_for_external_service(db: Session, external_service_id: int, skip: int = 0, limit: int = 100) -> List[Dependency]:
    ...
    if not external_service:
        logger.warning(f"Cannot get dependencies: ExternalService with ID {external_service_id} not found.")
        return []
        
    return db.query(Dependency).options(
        joinedload(Dependency.internal_system)
    ).filter(Dependency.external_service_id == external_service_id).order_by(Dependency.id.asc()).offset(skip).limit(limit).all()

def create_dependency(db: Session, internal_system_id: int, external_service_id: int, dependency_description: Optional[str] = None) -> Optional[Dependency]:
    """
    ...
    internal_system = get_internal_system(db, internal_system_id)
    if not internal_system:
        logger.error(f"Cannot create dependency: InternalSystem with ID {internal_system_id} not found.")
        return None

    external_service = get_external_service(db, external_service_id)
    if not external_service:
        logger.error(f"Cannot create dependency: ExternalService with ID {external_service_id} not found.")
        return None
    
    existing_dependency = db.query(Dependency).filter_by(
...
def update_dependency(
    ...
    updated = False
    if dependency_description is not None and db_dependency.dependency_description != dependency_description:
    ...
    if updated:
        try:
            ...
            db.rollback()
            logger.error(f"Error updating dependency ID {dependency_id}: {e}", exc_info=True)
            return None 
    else:
        logger.info(f"No update data (description) provided or data matches existing for dependency ID {dependency_id}.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(f"Created Notification: ID {new_notification.id}, Title: {new_notification.title}, Status: {new_notification.status.value}")
        logger.info(f"  RawEmail ID: {new_notification.raw_email_id}, Subject: {new_notification.raw_email_data.subject}")
        logger.info(f"  LLMData ID: {new_notification.llm_data_id}, Status: {new_notification.llm_data.processing_status.value}")

        # Test get_notification_by_original_email_id
        retrieved_notification = get_notification_by_original_email_id(db, test_email_id)
        assert retrieved_notification is not None
        assert retrieved_notification.id == new_notification.id
        logger.info(f"Retrieved notification by original email ID ({test_email_id}): ID {retrieved_notification.id}")
        ...
        if new_notification and new_notification.llm_data:
            ...
            updated_llm_data = update_llm_data_extracted_fields(
                db, llm_data_id=llm_data_id_to_update,
            ...
            logger.info(f"Updated LLMData ID {llm_data_id_to_update} with extracted fields. Notification status: {new_notification.status.value}")
            ...
            assert error_updated_llm_data.processing_status == ProcessingStatusEnum.ERROR
            db.refresh(new_notification) # Refresh to get updated notification status
            assert new_notification.status == NotificationStatusEnum.ERROR_PROCESSING
            logger.info(f"Updated LLMData ID {llm_data_id_to_update} status to ERROR. Notification status: {new_notification.status.value}")

        # Test get_notifications
        notifications_list = get_notifications(db, limit=5, options=[
            joinedload(Notification.raw_email_data),
            joinedload(Notification.llm_data)
        ])
        logger.info(f"Retrieved {len(notifications_list)} notifications via get_notifications:")
        for n in notifications_list:
            logger.info(f"  ID: {n.id}, Title: {n.title}, RawEmail Subject: {n.raw_email_data.subject if n.raw_email_data else 'N/A'}, LLM Status: {n.llm_data.processing_status.value if n.llm_data else 'N/A'}")
        ...
        pending_notification = create_notification(db, subject="Pending Email Test", received_at=datetime.utcnow(), original_email_id_str=pending_email_id)
        if pending_notification:
             logger.info(f"Created another notification ID {pending_notification.id} that should be pending initially.")
        ...
        for p_n in pending_list:
             logger.info(f"  Pending ID: {p_n.id}, LLMData Status: {p_n.llm_data.processing_status.value if p_n.llm_data else 'N/A'}")
