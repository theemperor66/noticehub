...
class LLMFactory:
    @staticmethod
    def get_llm_client(
        provider_name: str = None, api_key: str = None, model_name: str = None
    ) -> BaseLLM:
        ...
        if provider == "openai":
            ...
            gemini_model = (
                model_name  # Or a specific setting like settings.google_llm_model
            )
            if (
                provider == "gemini"
                and not gemini_model
                and "gemini" not in (settings.llm_model or "")
            ):
                ...
                gemini_model = (
                    settings.llm_model
                )  # Use general llm_model if it seems like a Gemini one

            try:
                return GeminiLLM(
                    api_key=api_key or settings.google_api_key, model_name=gemini_model
                )
            except ImportError as e:
                logger.error(
                    f"Failed to import GeminiLLM: {e}. Make sure 'google-generativeai' is installed."
                )
                raise
            except ValueError as e:
                logger.error(
                    f"ValueError for GeminiLLM: {e}. Check API key and model name."
                )
                raise
        ...
        #     return AnthropicLLM(api_key=api_key or settings.anthropic_api_key, model_name=model_name or settings.llm_model)
            ...
            raise ValueError(
                f"Unsupported LLM provider: {provider}. Supported: 'openai', 'google'/'gemini', 'groq'."
            )
...
if __name__ == "__main__":
    ...
    if settings.openai_api_key:
        try:
            ...
            logger.info(
                f"Successfully created OpenAI client: {openai_client.get_provider_name()}, Model: {openai_client.model_name}"
            )
        ...
        logger.warning(
            "OpenAI API key not set. Skipping OpenAI client test in factory."
        )
    ...
    if settings.google_api_key:
        try:
            ...
            gemini_client = LLMFactory.get_llm_client(
                provider_name="google"
            )  # Will use settings.llm_provider and settings.llm_model
            logger.info(
                f"Successfully created Google/Gemini client: {gemini_client.get_provider_name()}, Model: {gemini_client.model_name}"
            )
        ...
        logger.warning(
            "Google API key not set. Skipping Google/Gemini client test in factory."
        )

    # Test with an unsupported provider
    try:
        logger.info("Attempting to create an unsupported client...")
        unsupported_client = LLMFactory.get_llm_client(
