...
from src.data.models import (
    ...
    InternalSystem,
    Dependency,
)  # Removed create_tables, get_db_session as we'll make local ones
...
SAMPLE_EXTERNAL_SERVICES: List[Dict[str, Any]] = [
    {
        ...
        "provider": "AWS",
        "description": "AWS EC2 Compute in N. Virginia",
    },
    {
        "service_name": "AWS S3 Global",
        "provider": "AWS",
        "description": "AWS S3 Storage",
    },
    {
        "service_name": "Microsoft Azure VMs (East US)",
        "provider": "Microsoft Azure",
        "description": "Azure Virtual Machines in East US",
    },
    {
        "service_name": "Google Cloud Storage (multi-region)",
    ...
    },
    {
        ...
        "provider": "Slack",
        "description": "Team Collaboration and Messaging API",
    },
    {
        "service_name": "Twilio SMS API",
        "provider": "Twilio",
...
SAMPLE_INTERNAL_SYSTEMS: List[Dict[str, Any]] = [
    {
        "system_name": "Core Application Server",
        "responsible_contact": "app-dev-team@example.com",
        "description": "Main backend application for core business logic.",
    },
    {
        "system_name": "User Authentication Service",
        "responsible_contact": "auth-team@example.com",
        "description": "Handles user login, sessions, and SSO integration.",
    },
    {
        "system_name": "Data Processing Pipeline",
        "responsible_contact": "data-eng-team@example.com",
        "description": "ETL jobs for analytics and reporting.",
    },
    {
        "system_name": "Customer Notification Gateway",
        "responsible_contact": "comms-platform-team@example.com",
        "description": "Sends email, SMS, and push notifications to users.",
    },
    {
        "system_name": "Internal Wiki & Documentation",
        "responsible_contact": "knowledge-management@example.com",
        "description": "Confluence or similar internal documentation platform.",
    },
]
...
SAMPLE_DEPENDENCIES: List[Dict[str, Any]] = [
    {
        "internal_system_name": "Core Application Server",
        "external_service_name": "AWS EC2 us-east-1",
        "description": "Primary compute hosting for application logic.",
    },
    {
        "internal_system_name": "Core Application Server",
        "external_service_name": "AWS S3 Global",
        "description": "Stores user-generated content and static assets.",
    },
    {
        "internal_system_name": "User Authentication Service",
        "external_service_name": "AWS EC2 us-east-1",
        "description": "Hosted on EC2 instances for scalability.",
    },
    {
        "internal_system_name": "Data Processing Pipeline",
        "external_service_name": "AWS S3 Global",
        "description": "Reads source data from and writes processed data to S3.",
    },
    {
        "internal_system_name": "Data Processing Pipeline",
        "external_service_name": "Microsoft Azure VMs (East US)",
        "description": "Uses Azure VMs for specific Windows-based processing tasks.",
    },
    {
        "internal_system_name": "Customer Notification Gateway",
        "external_service_name": "Outlook 365",
        "description": "Relies on Outlook/Exchange for sending transactional emails.",
    },
    {
        "internal_system_name": "Customer Notification Gateway",
        "external_service_name": "Twilio SMS API",
        "description": "Sends SMS alerts and notifications via Twilio.",
    },
    {
        "internal_system_name": "Internal Wiki & Documentation",
        "external_service_name": "Google Cloud Storage (multi-region)",
        "description": "Backs up attachments and large files from the wiki.",
    },
]
...
def populate_data():
    ...
    logger.warning(
        f"USING TEMPORARILY HARDCODED DB URL FOR SEED SCRIPT: {HARDCODED_LOCAL_DB_URL.replace('noticehub_password', '********')}"
    )

    local_engine = create_engine(HARDCODED_LOCAL_DB_URL)
    logger.info(
        "Force dropping all tables in public schema with CASCADE before recreating..."
    )
    ...
    Base.metadata.create_all(
        local_engine
    )  # Ensure tables are created with the local engine
    ...
    try:
        ...
        for service_data in SAMPLE_EXTERNAL_SERVICES:
            service = crud.get_external_service_by_name(
                db, service_data["service_name"]
            )
            if not service:
                ...
                logger.info(
                    f"ExternalService '{service.service_name}' already exists. Skipping."
                )
            if service:
        ...
        for system_data in SAMPLE_INTERNAL_SYSTEMS:
            ...
            if not system:
                ...
                logger.info(
                    f"InternalSystem '{system.system_name}' already exists. Skipping."
                )
            if system:
        ...
        for dep_data in SAMPLE_DEPENDENCIES:
            ...
            if internal_system and external_service:
                ...
                logger.warning(
                    f"Could not create dependency for '{dep_data['internal_system_name']}' -> '{dep_data['external_service_name']}'. System or Service not found in created items."
                )
